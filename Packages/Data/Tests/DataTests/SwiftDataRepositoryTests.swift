...
/// SwiftData's store setup is not thread-safe: building in-memory containers
/// while other containers are being set up crashes inside CoreData's table
/// creation. Swift Testing parallelises suites, so every test that touches a
/// store lives under this serialised parent.
@Suite(.serialized)
