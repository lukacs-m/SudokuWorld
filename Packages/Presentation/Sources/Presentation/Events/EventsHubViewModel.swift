...
public final class EventsHubViewModel {
    public struct Content: Equatable, Sendable {
        public let daily: DailyLineup
        public let weekly: WeeklyTournament

        public init(daily: DailyLineup, weekly: WeeklyTournament) {
            self.daily = daily
            self.weekly = weekly
        }
    }
    ...
    public func load(now: Date = Date()) async {
        ...
        let daily = await getDailyLineup(dateKey: EventSeeds.dailyDateKey(for: now))
